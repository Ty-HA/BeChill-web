...
export default function WalletTester() {
  ...
  const generateMockData = (endpoint: string) => {
    ...
    // Portfolio endpoints
    if (endpoint.includes('/portfolio/') || endpoint === '/api/portfolio/:walletAddress') {
      return {
        mock: true,
    ...
    if (endpoint.includes('/token/trending')) {
      return {
        ...
        trending: [
          ...
          { symbol: "RNDR", price: 7.80, change24h: -2.1 },
    ...
    if (endpoint.includes('/transaction/')) {
      return {
        ...
        type: "Transfer",
        tokenTransfers: [
          { amount: 1.25, symbol: "SOL", usdValue: 125 }
        ]
      };
    }
  ...
  return (
    <div className="container mx-auto p-4 max-w-6xl mb-24"> {/* Add bottom margin to avoid footer overlap */}
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-purple-800">Wallet API Tester</h1>
        <div className="flex items-center">
          <span className="mr-2 text-sm">Use real API:</span>
          <button 
            onClick={() => setUseRealApi(!useRealApi)}
            className={`px-3 py-1 rounded-full ${useRealApi ? 'bg-green-500 text-white' : 'bg-gray-300 text-gray-700'}`}
          >
            {useRealApi ? 'ON' : 'OFF'}
          </button>
        </div>
      ...
      <div className="mb-6 p-4 bg-white rounded-lg shadow">
        <h2 className="text-xl font-semibold mb-2 text-purple-700">Wallet Address</h2>
        ...
        {walletAddress && !isValidWalletAddress() && (
          <p className="mt-2 text-red-500 text-sm">Wallet address should be between 32-44 characters</p>
        )}
      </div>

      {/* Portfolio Endpoints */}
      <div className="mb-6 p-4 bg-white rounded-lg shadow">
        <h2 className="text-xl font-semibold mb-4 text-purple-700">Portfolio Endpoints</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          <button
            onClick={() => callApi(() => apiService.getPortfolio(walletAddress), '/api/portfolio/:walletAddress')}
          ...
          <button
            onClick={() => callApi(() => apiService.getBalances(walletAddress), '/api/portfolio/balances/:walletAddress')}
            disabled={!isValidWalletAddress() || loading}
            className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
          >
            Get Balances
          ...
          <button
            onClick={() => callApi(() => apiService.getAssets(walletAddress), '/api/portfolio/assets/:walletAddress')}
            disabled={!isValidWalletAddress() || loading}
            className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
          >
            Get Assets
          ...
          <button
            onClick={() => callApi(() => apiService.getTransactionHistory(walletAddress), '/api/portfolio/history/:walletAddress')}
            disabled={!isValidWalletAddress() || loading}
            className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
          >
          ...
          <button
            onClick={() => callApi(() => apiService.getPortfolioAnalysis(walletAddress), '/api/portfolio/analysis/:walletAddress')}
            disabled={!isValidWalletAddress() || loading}
            className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
          >
          ...
          <button
            onClick={() => callApi(() => apiService.getTokenTransfers(walletAddress), '/api/portfolio/token-transfers/:walletAddress')}
            disabled={!isValidWalletAddress() || loading}
            className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
          >
            Get Token Transfers
      ...
      {/* Token Endpoints */}
      <div className="mb-6 p-4 bg-white rounded-lg shadow">
        <h2 className="text-xl font-semibold mb-4 text-purple-700">Token Endpoints</h2>
        
        {/* Token Address Input */}
        <div className="mb-4">
          <h3 className="text-md font-semibold mb-2 text-purple-600">Token Address</h3>
          <input
            type="text"
            value={tokenAddress}
          ...
          {tokenAddress && !isValidTokenAddress() && (
            <p className="mt-1 text-red-500 text-sm">Token address should be between 32-44 characters</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 mb-4">
          <button
            onClick={() => callApi(() => apiService.getToken(tokenAddress), '/api/token/:tokenAddress')}
          ...
          <button
            onClick={() => callApi(() => apiService.getTokenInfo(tokenAddress), '/api/token/info/:tokenAddress')}
            disabled={!isValidTokenAddress() || loading}
            className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
          >
            Get Token Info
          ...
          <button
            onClick={() => callApi(() => apiService.getTokenPrice(tokenAddress), '/api/token/price/:tokenAddress')}
            disabled={!isValidTokenAddress() || loading}
            className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
          >
            Get Token Price
          ...
          <button
            onClick={() => callApi(() => apiService.getTokenPriceHistory(tokenAddress), '/api/token/price-history/:tokenAddress')}
            disabled={!isValidTokenAddress() || loading}
            className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
          >
          ...
          <button
            onClick={() => callApi(() => apiService.getTokenLiquidity(tokenAddress), '/api/token/liquidity/:tokenAddress')}
            disabled={!isValidTokenAddress() || loading}
            className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
          >
            Get Token Liquidity
        ...
        {/* Symbol Input */}
        <div className="mb-4">
          <h3 className="text-md font-semibold mb-2 text-purple-600">Token Symbol</h3>
          <div className="flex gap-2">
            <input
              type="text"
              value={tokenSymbol}
            ...
            />
            <button
              onClick={() => callApi(() => apiService.getTokenMarketData(tokenSymbol), '/api/token/market-data/:symbol')}
        ...
        {/* Compare Tokens */}
        <div className="mb-4">
          <h3 className="text-md font-semibold mb-2 text-purple-600">Compare Tokens</h3>
          <div className="flex gap-2">
            <input
              type="text"
              value={compareTokens}
            ...
            />
            <button
              onClick={() => callApi(() => apiService.compareTokens(compareTokens.split(',')), '/api/token/compare')}
        ...
        {/* Trending Tokens */}
        <button
          onClick={() => callApi(() => apiService.getTrendingTokens(), '/api/token/trending')}
          disabled={loading}
          className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
        >
          Get Trending Tokens
      ...
      {/* Transaction Endpoint */}
      <div className="mb-6 p-4 bg-white rounded-lg shadow">
        <h2 className="text-xl font-semibold mb-4 text-purple-700">Transaction Endpoint</h2>
        <div className="flex gap-2">
          <input
            type="text"
            value={txSignature}
          ...
          />
          <button
            onClick={() => callApi(() => apiService.getTransaction(txSignature), '/api/transaction/:signature')}
        ...
        {txSignature && !isValidSignature() && (
          <p className="mt-2 text-red-500 text-sm">Transaction signature should be at least 32 characters</p>
        )}
      </div>
