...
export default function ChatComponent({
  ...
  const handleWalletDetected = () => {
    ...
    setTimeout(() => {
      addMessage({
        text: "Done! Ask about asset breakdown, recent transactions, or investment ideas.",
        sender: "bot",
      });

      setTimeout(() => {
        addMessage({
          text: "Example: 'What's my wallet performance last month?'",
          sender: "bot",
        });
      }, 1500);
    }, 2000);
  ...
  const suggestWalletConnection = () => {
    if (!userWallet && !hasAskedForWallet) {
      addMessage({
        text: "To give you insights, I'd need to connect your wallet. Connect now?",
        sender: "bot",
      });
  ...
  const handleSendMessage = () => {
    ...
    if (hasAskedForWallet && !userWallet) {
      if (/yes|connect|sure|okay/i.test(userText)) {
        ...
        addMessage({
          text: "No worries! Ask me anything crypto.",
          sender: "bot",
        });
        return;
      }
  ...
  const renderWalletReviewWidget = () => (
    <div className="bg-purple-600 p-4 rounded-xl w-60 my-2 text-center">
      ...
      <button
        ...
        onClick={() => setShowWalletStory(true)}
      >
        VIEW STORY
      </button>
    </div>
